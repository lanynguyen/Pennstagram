...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        int w = pic.getWidth();
        int h = pic.getHeight();
        int hNew = h + (borderWidth * 2);
        int wNew = w + (borderWidth * 2);

        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[hNew][wNew]; // extend image size

        for (int col = 0; col < wNew; col++) {
            for (int row = 0; row < hNew; row++) {
                if (row < borderWidth || col < borderWidth || row >= (h + borderWidth)
                        || col >= (w + borderWidth)) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                bmp[row][col] = new Pixel(
                        Math.abs(rInverted), Math.abs(gInverted), Math.abs(bInverted)
                );
            }
        }
        return new PixelPicture(bmp);
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int g = p.getGreen();
                int b = p.getBlue();
                int rScaled = (int) Math.round(rfactor * r);
                int gScaled = (int) Math.round(gfactor * g);
                int bScaled = (int) Math.round(bfactor * b);
                bmp[row][col] = new Pixel(rScaled, gScaled, bScaled);
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int col = 0; col < w1; col++) {
            for (int row = 0; row < h1; row++) {
                ...
                bmp1[row][col] = new Pixel(rAvg, gAvg, bAvg);
            }
        }
