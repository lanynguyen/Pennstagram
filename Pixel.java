...
public class Pixel implements Comparable<Pixel> {
    ...
    public Pixel(int[] c) {
        ...
        int[] newArray = { 0, 0, 0 };
        for (int i = 0; i < Math.min(c.length, 3); i++) {
            newArray[i] = c[i];
        }

        red = clip(newArray[0]);
        green = clip(newArray[1]);
        blue = clip(newArray[2]);
    }
    ...
    public int[] getComponents() {
        int[] array = { this.getRed(), this.getGreen(), this.getBlue() };
        return array;
    }
    ...
    public int distance(Pixel px) {
        ...
        return Math.abs(this.getRed() - px.getRed()) + Math.abs(this.getGreen() - px.getGreen()) +
                Math.abs(this.getBlue() - px.getBlue());
    }
    ...
    public boolean sameRGB(Pixel px) {
        return this.getRed() == px.getRed() && this.getBlue() == px.getBlue()
                && this.getGreen() == px.getGreen();
    }
