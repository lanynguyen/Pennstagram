...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int g = p.getGreen();
                int b = p.getBlue();
                int rContrasted = (int) Math.round(((r - avg) * multiplier) + avg);
                int gContrasted = (int) Math.round(((g - avg) * multiplier) + avg);
                int bContrasted = (int) Math.round(((b - avg) * multiplier) + avg);
                bmp[row][col] = new Pixel(rContrasted, gContrasted, bContrasted);
            }
        }
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        Pixel[] sorted = m.getSortedPixels();

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                Pixel palette = sorted[0];
                int dist = p.distance(palette);
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int left = Math.max(0, col - radius);
                int right = Math.min(w, col + radius + 1);
                int top = Math.max(0, row - radius);
                int bottom = Math.min(h, row + radius + 1);
                int count = 0;
                ...
                tgt[row][col] = new Pixel(rAvg, gAvg, bAvg);
            }
        }
